			...
			try:
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						...
						elif msg.get("text").startswith("ربات") or msg.get("text").startswith("بات") or msg.get("text").startswith("بلکترون") or msg.get("text").startswith("روبات") or msg.get("text").startswith("tik") or msg.get("text").startswith("tik"):
							try:
                    ...
                    elif msg.get("text").startswith("کوص ننت"):
						bot.sendMessage(target, "برو کیرم تو دهن ننت", message_id=msg["message_id"])

                        elif msg.get("text").startswith("تایپر ها چه کسانی هستن"):
    						bot.sendMessage(target, "تایپر ها ی بچه سال گنده گوز هستن", message_id=msg["message_id"])

                            elif msg.get("text").startswith("خوبین"):
    						bot.sendMessage(target, "خوبم تو خوبی", message_id=msg["message_id"])

                 elif msg.get("text").startswith("خوبم"):
						bot.sendMessage(target, "شکر خدا همیشه خوب باشید", message_id=msg["message_id"])

                        elif msg.get("text").startswith("سازنده"):
    						bot.sendMessage(target, "مدیر گپ", message_id=msg["message_id"])

                        elif msg.get("text").startswith("بمیرم"):
						    bot.sendMessage(target, "بکیرم بمیر", message_id=msg["message_id"])

                        elif msg.get("text").startswith("ربات"):
						    bot.sendMessage(target, "بنال", message_id=msg["message_id"])

                        elif msg.get("text").startswith("لف میدیم"):
						    bot.sendMessage(target, "بکیرمون لف بده", message_id=msg["message_id"])

                            elif msg.get("text").startswith("هویاااا"):
    						bot.sendMessage(target, "عشقم بد رفته توت", message_id=msg["message_id"])

                            elif msg.get("text").startswith("اصل"):
    						bot.sendMessage(target, "ربات 1000ساله امیرکا", message_id=msg["message_id"])

                            elif msg.get("text").startswith("ممنون"):
    						bot.sendMessage(target, "خواهش میکنم", message_id=msg["message_id"])

                            elif msg.get("text").startswith("نخبر"):
    						bot.sendMessage(target, "سلامتی", message_id=msg["message_id"])

                            elif msg.get("text").startswith("اصل بدین"):
    						bot.sendMessage(target, "فیک بدیم", message_id=msg["message_id"])

                            elif msg.get("text").startswith("ربات"):
    						bot.sendMessage(target, "گونخور دیگه چقدر صدام میکنی", message_id=msg["message_id"])

                            elif msg.get("text").startswith("من هکرم"):
    						bot.sendMessage(target, "تو لمری", message_id=msg["message_id"])

                            elif msg.get("text").startswith("من تایپرم"):
    						bot.sendMessage(target, "تو لمری", message_id=msg["message_id"])

                            elif msg.get("text").startswith("کانال"):
    						bot.sendMessage(target, "@robot_000", message_id=msg["message_id"])

                            elif msg.get("text").startswith("لف ندین"):
    						bot.sendMessage(target, "میدونی اینا با این کارشون فقط کون میدن", message_id=msg["message_id"])

                            elif msg.get("text").startswith("قوانین"):
    						bot.sendMessage(target, "لینک ممنوغ........فحاشی ممنوع.....گوخوری ممنوع.....شاخ بازی ممنوع....ادم باشید", message_id=msg["message_id"])

                            elif msg.get("text").startswith("جق"):
    						bot.sendMessage(target, "جق جق نا پیروزی", message_id=msg["message_id"])

                            elif msg.get("text").startswith("تیتاب"):
    						bot.sendMessage(target, "مصل اینه که به سگ تیتاب دادن", message_id=msg["message_id"])

                            elif msg.get("text").startswith("کیر"):
    						bot.sendMessage(target, "هسته خرما", message_id=msg["message_id"])

                            elif msg.get("text").startswith("کیرم دهنت"):
    						bot.sendMessage(target, "کیر کیر نکن بده ننت بخوره", message_id=msg["message_id"])

                            elif msg.get("text").startswith("سلوم"):
    						bot.sendMessage(target, "سلوم دلبر", message_id=msg["message_id"])

                            elif msg.get("text").startswith("کص ننت"):
    						bot.sendMessage(target, "کص ببیت", message_id=msg["message_id"])

                            elif msg.get("text").startswith("ننت گایدم"):
    						bot.sendMessage(target, "برو بچه تا کص ننت نزاشتم", message_id=msg["message_id"])

                            elif msg.get("text").startswith("دعوا نکنین"):
    						bot.sendMessage(target, "بله حق با ایشونه دعوا نکنین", message_id=msg["message_id"])

                            elif msg.get("text").startswith("خایمال"):
    						bot.sendMessage(target, "خایمال سگ گاید", message_id=msg["message_id"])

                            elif msg.get("text").startswith("فیلم"):
    						bot.sendMessage(target, "خجالت بکش مردک جومونگ نما", message_id=msg["message_id"])

                            elif msg.get("text").startswith("عشقم"):
    						bot.sendMessage(target, "وای با من بودی زوق کردم دلبر", message_id=msg["message_id"])

                            elif msg.get("text").startswith("بیاین تو گپم"):
    						bot.sendMessage(target, "هرکی بره لاشیه", message_id=msg["message_id"])

                            elif msg.get("text").startswith("من اومدم"):
    						bot.sendMessage(target, "خوش اومدی گل", message_id=msg["message_id"])

                            elif msg.get("text").startswith("عشقتون اومد"):
    						bot.sendMessage(target, "ای جان عشقمون اومد ی دست بزن براش", message_id=msg["message_id"])

                            elif msg.get("text").startswith("تیتاب طلایی"):
    						bot.sendMessage(target, "تیتاب ندیده بدبخت", message_id=msg["message_id"])

                            elif msg.get("text").startswith("اهنگ بخونم"):
    						bot.sendMessage(target, "نه ترو خدا نخون حالم بد میشه", message_id=msg["message_id"])

                            elif msg.get("text").startswith("خدافظ"):
    						bot.sendMessage(target, "میموندی برات چایی میاوردم", message_id=msg["message_id"])

                            elif msg.get("text").startswith("فعلا"):
    						bot.sendMessage(target, "زود بیا", message_id=msg["message_id"])

                            elif msg.get("text").startswith("بای"):
    						bot.sendMessage(target, "خارجی حرف میزنی مردک", message_id=msg["message_id"])

                            elif msg.get("text").startswith("حوصلم"):
    						bot.sendMessage(target, "منم", message_id=msg["message_id"])

                            elif msg.get("text").startswith("دلبر"):
    						bot.sendMessage(target, "جان دلم", message_id=msg["message_id"])
