			...
			try:
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						if hasAds(msg.get("text")) and not msg.get("author_object_guid") in admins :
						...
						elif msg.get("text") == "ربات خاموش" or msg.get("text") == "/stop" and msg.get("author_object_guid") in admins :
							try:
								sleeped = True
							...
							try:
								...
								if guid in blacklist:
									if msg.get("author_object_guid") in admins:
										...
										bot.sendMessage(target, "❌ داداش کاربر جنده محدود میباشد", message_id=msg.get("message_id"))
								else:
									bot.invite(target, [guid])
							...
							try:
								msg.get("txt") == "جرعت و حقیت دوم"
								rules = open("game.txt","w",encoding='utf-8').write(str(msg.get("text").strip("اپدیت بازی")))
								bot.sendMessage(target, "✅  بازی اپدیت شد", message_id=msg.get("message_id"))
								# rules.close()
							except:
								bot.sendMessage(target, "❌ لطفا دستور را به درستی وارد کنید", message_id=msg.get("message_id"))
						...
						elif msg.get("text") == "داستان":
							try:
								rules = open("dok.txt","r",encoding='utf-8').read()
								bot.sendMessage(target, str(rules), message_id=msg.get("message_id"))
							except:
								print("err dastorat")
								
						elif msg.get("text") == "جوک":
							try:
							...
							try:
								rules = open("rules.txt","w",encoding='utf-8').write(str(msg.get("text").strip("اپدیت دستورات")))
								bot.sendMessage(target, "اپدیت دستورات انجام شد", message_id=msg.get("message_id"))
								# rules.close()
							except:
								bot.sendMessage(target, "❌ لطفا دستور را به درستی وارد کنید", message_id=msg.get("message_id"))
						...
						elif msg.get("text") == "ربات کی کونیه":
							try:
								rules = open("robot.txt","r",encoding='utf-8').read()
								bot.sendMessage(target, str(rules), message_id=msg.get("message_id"))
							except:
								print("err ghanon")
								
						elif msg.get("text") == "بیوگرافی":
							try:
								rules = open("bio.txt","r",encoding='utf-8').read()
								bot.sendMessage(target, str(rules), message_id=msg.get("message_id"))
							except:
								print("err ghanon")
							...
							try:
								rules = open("boti.txt","r",encoding='utf-8').read()
								bot.sendMessage(target, str(rules), message_id=msg.get("message_id"))
							except:
								print("err ghanon")
						...
						if msg.get("text") == "ربات روشن" or msg.get("text") == "/start" and msg.get("author_object_guid") in admins :
							try:
								sleeped = False
					...
					if data["type"]=="RemoveGroupMembers":
						try:
							...
							bot.sendMessage(target, f"دیدی کوبص پدرت گزاشتم خخخخ {user}", message_id=msg["message_id"])
						...
						try:
							...
							bot.sendMessage(target, f"لف دادن  کون نیس که هر دفعه میدی جنده {user}", message_id=msg["message_id"])
							# bot.deleteMessages(target, [msg["message_id"]])
						except:
