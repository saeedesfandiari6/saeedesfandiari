...
while True:
	...
	#				bot.sendMessage(target, "یک پیام مستهجن پاک شد ✅", message_id=msg.get("message_id"))
				...
				if data["type"]=="RemoveGroupMembers":
					user = bot.getUserInfo(data['peer_objects'][0]['object_guid'])["data"]["user"]["first_name"]
					bot.sendMessage(target, f"دیدی کون پدرت گزاشتم {user}", message_id=msg["message_id"])
					...
					bot.sendMessage(target, f"منم دوست دارم ولی چرا لف دادی کصکش", message_id=msg["message_id"])
